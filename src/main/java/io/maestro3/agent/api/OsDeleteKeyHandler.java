...
@Component("osDeleteKeyHandler")
public class OsDeleteKeyHandler implements IM3ApiHandler {

    protected final Logger LOG = LoggerFactory.getLogger(OsDeleteKeyHandler.class);
    ...
    @Autowired
    public OsDeleteKeyHandler(IOpenStackRegionRepository regionDbService, TenantDbService tenantDbService,
    ...
    public M3RawResult handle(M3ApiAction action) throws M3PrivateAgentException {
        DeleteKeyRequest request = M3ApiActionInverter.toDeleteKeyRequest(action);
...
String name = request.getName();
    ...
    private void deleteKey(OpenStackRegionConfig region, OpenStackTenant tenant,
                           String name) throws M3PrivateAgentException {
        try {
    ...
    public Set<ActionType> getSupportedActions() {
        return new HashSet<>(Collections.singletonList(ActionType.DELETE_KEY));
    }
}

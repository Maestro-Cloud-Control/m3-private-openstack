...
@Component("osStopInstanceHandler")
public class OsStopInstanceHandler extends AbstractInstanceHandler implements IM3ApiHandler {

    @Autowired
    public OsStopInstanceHandler(IOpenStackRegionRepository regionDbService, TenantDbService tenantDbService,
    ...
    public M3RawResult handle(M3ApiAction action) throws M3PrivateAgentException {
        StopInstanceRequest request = M3ApiActionInverter.toStopInstanceRequest(action);
...
OpenStackServerConfig server = serverDbService.findServer(region.getId(), tenant.getId(), request.getInstanceId());
    ...
    private void stopServer(OpenStackRegionConfig region,
                            OpenStackTenant tenant,
                            OpenStackServerConfig server) throws M3PrivateAgentException {
        try {
            openStackApiProvider.openStack(tenant, region).compute().servers().stop(server.getNativeId());
    ...
    public Set<ActionType> getSupportedActions() {
        return new HashSet<>(Collections.singletonList(ActionType.STOP_INSTANCE));
    }
}

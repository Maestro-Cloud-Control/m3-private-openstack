...
@Component("OsDeleteTagsHandler")
public class OsDeleteTagsHandler extends AbstractInstanceHandler implements IM3ApiHandler {

    @Autowired
    public OsDeleteTagsHandler(IOpenStackRegionRepository regionDbService, TenantDbService tenantDbService,
    ...
    public M3RawResult handle(M3ApiAction action) throws M3PrivateAgentException {
        DeleteTagsRequest request = JsonUtils.parseMap(action.getParams(), DeleteTagsRequest.class);
        ...
        TagUtils.removeTagsByKeys(server, TagUtils.extractTagKeys(request.getTags()));
    ...
    public Set<ActionType> getSupportedActions() {
        return new HashSet<>(Collections.singletonList(ActionType.DELETE_TAGS));
    }
}

...
@Component("osStartInstanceHandler")
public class OsStartInstanceHandler extends AbstractInstanceHandler implements IM3ApiHandler {

    @Autowired
    public OsStartInstanceHandler(IOpenStackRegionRepository regionDbService, TenantDbService tenantDbService,
    ...
    public M3RawResult handle(M3ApiAction action) throws M3PrivateAgentException {
        StartInstanceRequest request = M3ApiActionInverter.toStartInstanceRequest(action);
...
OpenStackServerConfig server = serverDbService.findServer(region.getId(), tenant.getId(), request.getInstanceId());
    ...
    private void startServer(OpenStackRegionConfig region,
    ...
    public Set<ActionType> getSupportedActions() {
        return new HashSet<>(Collections.singletonList(ActionType.START_INSTANCE));
    }
}
